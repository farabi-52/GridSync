...
def build_demand_profiles(
    ...
    if isinstance(scale, dict):
        ...
        for country, scale_country in scale.items():
            gegis_load.loc[
                gegis_load.region_code == country, "demand"
            ] *= scale_country

    elif isinstance(scale, (int, float)):
        logger.info(f"Load data scaled with scaling factor {scale}.")
        # Handle different possible column names
        if "demand" in gegis_load.columns:
            gegis_load["demand"] *= scale
        else:
    # Print what columns we actually have
            logger.info(f"Available columns in gegis_load: {gegis_load.columns.tolist()}")
    
    # Try common alternative column names
            possible_cols = ['Electricity demand','Electricity Demand','load', 'Load', 'DEMAND', 'Demand', 'electricity', 'power']
            found = False
    
    for col in possible_cols:
        if col in gegis_load.columns:
            logger.info(f"Using column '{col}' instead of 'demand'")
            gegis_load[col] *= scale
            gegis_load.rename(columns={col: "demand"}, inplace=True)
            found = True
            break
    
    if not found:
        raise KeyError(f"Could not find demand column. Available columns: {gegis_load.columns.tolist()}")
    ...
    def upsample(cntry, group):
        ...
        """
        l = gegis_load.loc[gegis_load.region_code == cntry]["demand"]
        if len(group) == 1:
