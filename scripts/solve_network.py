...
import pypsa
import xarray as xr
from add_hydrogen import add_h2_sector  
...
# ── StorageUnit safe accessor: split/signed/none ─────────────────────────────
# ---- helper: robust access to StorageUnit split variables --------------------
def get_storage_split(n):
    """
    Return (p_dis, p_ch) for StorageUnits with dims ('snapshot','name').
    Works if the model uses split variables (p_dispatch/p_store) or a signed p;
    returns empty arrays if there are no StorageUnits at all.
    """
    m = n.model
    # 1) split-form: p_dispatch / p_store
    try:
        p_dis = m["StorageUnit-p_dispatch"].loc[n.snapshots]
        p_ch  = m["StorageUnit-p_store"].loc[n.snapshots]
        return p_dis, p_ch
    except KeyError:
        pass
    # 2) signed-form: single 'p' that we split into +/-
    try:
        p = m["StorageUnit-p"]
        p_dis = p.where(p > 0, 0)
        p_ch  = (-p).where(p < 0, 0)
        ...
        # 3) no StorageUnits at all → return empty 2D arrays (snapshots × 0 names)
        import numpy as np
        import xarray as xr
        empty = xr.DataArray(
            np.zeros((len(n.snapshots), 0)),
            coords={"snapshot": n.snapshots, "name": []},
            dims=("snapshot", "name"),
        )
        return empty, empty
# -----------------------------------------------------------------------------
...
def add_RES_constraints(n, res_share, config):
    ...
    lhs_gen = (
        ...
        # StorageUnits (robust to any formulation)
    p_dis_SU, p_ch_SU = get_storage_split(n)

    # intersect with available StorageUnit names to avoid KeyError
    try:
        su_names = set(p_dis_SU.coords["name"].values.tolist())
    except Exception:
        su_names = set()
    stores_i_ok = [s for s in stores_i if s in su_names]

    if stores_i_ok:
        store_disp_expr = p_dis_SU.loc[:, stores_i_ok] * stores_t_weights
        store_expr      = p_ch_SU .loc[:, stores_i_ok] * stores_t_weights
        lhs_dispatch = store_disp_expr.groupby(sgrouper).sum()
        lhs_store    = store_expr.groupby(sgrouper).sum()
    else:
        # no StorageUnits present/selected → zero contribution
        lhs_dispatch = 0
        lhs_store    = 0

    # Link-based batteries / electrolysers
    charge_expr = n.model["Link-p"].loc[:, charger_i] * stores_t_weights.apply(
        lambda r: r * n.links.loc[charger_i].efficiency
    )
    if not charger_i.empty:
        charge_expr = n.model["Link-p"].loc[:, charger_i] * stores_t_weights.apply(
            lambda r: r * n.links.loc[charger_i].efficiency
        )
...
if __name__ == "__main__":
    ...
    if snakemake.config.get("sector_coupling", {}).get("h2_fertilizer", False):
        add_h2_sector(n)
    n = solve_network(
        n,
